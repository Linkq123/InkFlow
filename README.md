...
当前版本：`0.1.1`（Windows x64 v1）。项目不包含账号、遥测、后台联网、云同步、AI、插件系统或 Git 操作。

## 已实现

- Tauri 2 + Rust + WebView2 桌面外壳，保留 Windows 原生窗口边框、缩放与 Snap。
- Svelte 5 + CodeMirror 6 编辑器，支持实时融合、源码和只读预览三种模式；切换标签或预览模式后保留各文档撤销历史，隐藏标签只缓存选择与历史而不复制完整正文；保存引起的资源路径改写按具体图片出现位置同步重建等价的撤销/重做分支，代码示例中的同名路径保持原样；图片上传成功或失败时也只重放对应占位符编辑，不混入用户编辑步骤。
...
生产构建使用共享的 ES Module Worker；Markdown 渲染和远程图片提示检测均离开输入主线程，后者在停止输入 180ms 后才调度。没有图片语法候选的文档会在 Worker 内直接返回，其余文档复用同一套净化渲染管线，并且不会重复打包解析依赖。同类型新请求会终止仍在执行的旧 Worker 任务，另一类型的有效请求会在重建后的 Worker 中自动重放，避免过期全文解析积压。原始 HTML、KaTeX、Mermaid 和代码围栏语言包按内容懒加载。发布产物不携带源码映射；首屏编辑代码与大型可选渲染依赖保持在不同分包中。
