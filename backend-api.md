...
- 清理按每文档 50 个、30 天和 500MB 全局上限执行；已成功删除的淘汰记录不再占用全局配额，因文件锁或权限问题暂时无法删除的记录仍按实际大小参与后续配额计算。解压与枚举在 Rust 阻塞任务线程执行，前端只在首屏可交互后进行后台检查或打开恢复中心时等待，不阻塞窗口和文件关联启动。
...
- 会话写入本地应用数据目录的 `session.json`，使用原子写入。前端在标签、活动文档、模式或工作区变化后延迟 350ms 串行写入；瞬时失败最多按 1、2、3 秒退避重试三次，且新状态会取代旧状态的待重试任务。窗口关闭前尝试刷入最新快照；会话写入失败不会阻止用户关闭应用。
- 正常启动单独打开并立即呈现活动文档，再恢复工作区，并以每批最多两个路径恢复其余标签。文件关联或进程启动参数中的显式路径优先于旧会话；初始化期间由第二实例送达的路径会在会话恢复结束后排空，缺失文件和不可访问工作区会被跳过而不会阻止启动。
