...
- 表格悬浮命令支持增加/删除行列，包括带 1–3 个前导空格的合法表格；按表头列号处理省略尾部空单元格的行，并按 Markdown 块边界保留后续标题、列表等正文。选区工具条、`/` 块命令和统一命令面板；斜杠命令只替换光标前的命令文本，保留右侧原文。
...
- 图片迁移对带引号的 `srcset` 也编码路径中的空白和逗号；引用标签保留反斜杠及字符引用的区别，避免并发另存为漏改定义。复制图片采用 50 MiB 有界读取，超限或临时图片缺失时拒绝保存并回滚新建资源。
